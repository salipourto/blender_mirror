...
HIPRT_DEVICE constexpr T&& forward( typename remove_reference<T>::type&& t ) noexcept
{
	static_assert( !is_lvalue_reference<T>::value, "template argument T must not be an lvalue reference type" );
	return static_cast<T&&>( t );
}
...
class hiprtPimpl
{
  ...
  public:
	...
	HIPRT_DEVICE static constexpr void PIMPL_SIZE_CHECK_()
	{
	...
	HIPRT_DEVICE static constexpr void PIMPL_PTR_CHECK_()
	{
		...
		static_assert(
			alignof( T ) >= hiprt::alignment_of<T>::value, "hiprtPimpl: T has a more restrict alignment than given" );
	}
