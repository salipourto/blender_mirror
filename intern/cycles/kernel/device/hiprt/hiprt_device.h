 ...
 */
typedef bool (*hiprtIntersectFunc)(const hiprtRay &ray,
                                   uint32_t instanceID,
                                   uint32_t primID,
                                   const void *data,
                                   void *payload,
 ...
 * \return A flag indicating hit.
 */
typedef bool (*hiprtFilterFunc)(const hiprtRay &ray,
                                uint32_t instanceID,
                                uint32_t primID,
                                const void *data,
                                void *payload,
 ...
 *
 */
template<uint32_t PrivateStackSize> class hiprtCustomPrivateStack {
 public:
...
class hiprtCustomSharedStack {
 public:
  HIPRT_DEVICE hiprtCustomSharedStack(int *globalStackBuffer,
                                      u32 globalStackSize,
...
template<typename hiprtStack> class hiprtGeomTraversalClosestCustomStack {
 public:
  HIPRT_DEVICE hiprtGeomTraversalClosestCustomStack(hiprtGeometry geom,
                                                    const hiprtRay &ray,
                                                    hiprtStack &stack);
  HIPRT_DEVICE hiprtHit getNextHit();
  HIPRT_DEVICE hiprtTraversalState getCurrentState();
...
template<typename hiprtStack> class hiprtGeomTraversalAnyHitCustomStack {
 public:
  HIPRT_DEVICE hiprtGeomTraversalAnyHitCustomStack(hiprtGeometry geom,
                                                   const hiprtRay &ray,
                                                   hiprtStack &stack);
  HIPRT_DEVICE hiprtHit getNextHit();
  HIPRT_DEVICE hiprtTraversalState getCurrentState();
...
template<typename hiprtStack> class hiprtSceneTraversalClosestCustomStack {
 public:
  HIPRT_DEVICE hiprtSceneTraversalClosestCustomStack(hiprtScene scene,
                                                     const hiprtRay &ray,
                                                     hiprtRayMask mask,
                                                     hiprtStack &stack);
  HIPRT_DEVICE hiprtSceneTraversalClosestCustomStack(hiprtScene scene,
...
template<typename hiprtStack> class hiprtSceneTraversalAnyHitCustomStack {
 public:
  HIPRT_DEVICE hiprtSceneTraversalAnyHitCustomStack(hiprtScene scene,
                                                    const hiprtRay &ray,
                                                    hiprtRayMask mask,
                                                    hiprtStack &stack);
  HIPRT_DEVICE hiprtSceneTraversalAnyHitCustomStack(hiprtScene scene,
